...
| Action ambiguity | Refund amount or target is missing | block execution until an explicit amount or target is present | ambiguity tests |
...
| Concurrency | Duplicate approval, execution, or audit sequence | atomic review claim and serialized SQLite audit append; production connectors still need external idempotency keys | concurrent tests |
